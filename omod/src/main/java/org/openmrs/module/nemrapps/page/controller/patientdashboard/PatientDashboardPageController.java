 ...
 */
package org.openmrs.module.nemrapps.page.controller.patientdashboard;
...
public class PatientDashboardPageController {
	...
	public Object controller(@RequestParam("patientId") Patient patient,
	        ...
	        @InjectBeans PatientDomainWrapper patientDomainWrapper, @SpringBean("orderService") OrderService orderService,
		...
		// Create context model for overall actions(exclude the active visit from the context to make general actions independent of an active visit)
		AppContextModel overallActionsContextModel = (AppContextModel) contextModel.clone();
		if (activeVisit != null) {
			overallActionsContextModel.remove("visit");
		}
		model.addAttribute("overallActionsContextModel", overallActionsContextModel);
